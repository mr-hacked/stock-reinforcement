    ...
    with open(user_file) as f1:
        ...
        for user_info in all_user_info:
            twitter_user_ob = TwitterUser(re.split(r'\t+', user_info)[0], re.split(r'\t+', user_info)[1],
                                          ...
                                          float(re.split(r'\t+', user_info)[6]),
                                          float(re.split(r'\t+', user_info)[7][:-1]))
...
if os.path.isfile("training_labels.dat") and os.path.isfile("training_features_matrix.dat") and os.path.isfile(
        "testing_labels.dat") and os.path.isfile("testing_features_matrix.dat"):
    training_labels = pickle.load(open("training_labels.dat", "rb"))
    training_features = pickle.load(open("training_features_matrix.dat", "rb"))
    testing_labels = pickle.load(open("testing_labels.dat", "rb"))
    testing_features = pickle.load(open("testing_features_matrix.dat", "rb"))
    ...
    training_labels = [0] * len(training_spammers_feature_matrix) + [1] * len(
        training_legit_feature_matrix)  # 0 spammers, 1 - legit
...
def select_classifier(algo, label):
    ...
    plt.figure()
    ax = plot_confusion_matrix(metrics.confusion_matrix(expected, predicted), label)

    return metrics, ax


# Initial Model
initial = select_classifier(GaussianNB, "Naive Bayes Classifier")
save(initial, 'model_i')

# Naive Bayes
final = select_classifier(GaussianNB(), "Naive Bayes Classifier")
save(final, 'model_f')
